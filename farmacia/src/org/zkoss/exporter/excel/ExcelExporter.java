...
public class ExcelExporter extends AbstractExporter<XSSFWorkbook, Row> {
    ...
    @Override
    protected void exportTabularComponent(MeshElement component, OutputStream outputStream, String cnombre) throws Exception {
        XSSFWorkbook book = new XSSFWorkbook();
        setExportContext(new ExportContext(true, book.createSheet(cnombre)));
    ...
    @Override
    protected void exportDataModel(String[] headers, String[] fields, ListModelList dataModel, OutputStream outputStream, String cnombre) throws Exception {
        XSSFWorkbook book = new XSSFWorkbook();
        setExportContext(new ExportContext(true, book.createSheet(cnombre)));
        //LISTBOX
        Listbox lstExportar = new Listbox();
        lstExportar.setMold("paging");
        lstExportar.setPageSize(5000);
        Listhead cabeceras = new Listhead();
        cabeceras.setParent(lstExportar);
        Listheader cabecera;
        for (int i = 0; i < headers.length; i++) {
            cabecera = new Listheader(headers[i]);
            cabecera.setParent(cabeceras);
        }

        lstExportar.setModel(dataModel);
        lstExportar.setItemRenderer(new ListitemRedenderZarcillo(fields));
        lstExportar.onInitRender();
        //LISTBOX
        int columnSize = getHeaderSize(lstExportar);
        exportHeaders(columnSize, lstExportar, book);
        exportRows(columnSize, lstExportar, book);
        exportFooters(columnSize, lstExportar, book);
